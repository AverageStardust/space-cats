...
func bind_external_function(
	...
	_story.bind_external_function(func_name, method, lookahead_safe)
...
func evaluate_function(function_name: String, arguments = []) -> InkFunctionResult:
	...
	var result = await _story.evaluate_function(function_name, arguments, true)
...
func _create_story(json_story, runtime) -> void:
	_story = await InkStory.new(json_story, runtime)


func _async_create_story(json_story, runtime) -> void:
	await _create_story(json_story, runtime)
