...
func continue_async(millisecs_limit_async: float):
	...
	await continue_internal(millisecs_limit_async)
...
func continue_internal(millisecs_limit_async: float = 0) -> void:
	...
	while (first_time || self.can_continue):
		...
		output_stream_ends_in_newline = await self.continue_single_step()
...
func continue_story_maximally() -> String:
	...
	while (self.can_continue):
		_str += await self.continue_story()
		if self._ink_runtime.has_raised_uncaught_exceptions:
...
func step() -> void:
	...
	var is_logic_or_flow_control = await perform_logic_and_flow_control(current_content_obj)
...
func perform_logic_and_flow_control(content_obj: InkObject) -> bool:
	...
	if InkUtils.is_ink_class(content_obj, "Divert"):
		...
		if current_divert.has_variable_target:
			...
			await call_external_function(current_divert.target_path_string, current_divert.external_args)
			return true
		else:
...
func evaluate_function(
	...
	var string_output = ""
	while self.can_continue:
		string_output += await self.continue_story()
...
func call_external_function(func_name: String, number_of_arguments: int) -> void:
	...
	var func_result = await _func_def.execute(arguments)
...
func bind_external_function_general(
	...
	_externals[func_name] = ExternalFunctionDef.new(method, lookahead_safe)


# try_coerce not needed.


# (String, Callable, bool) -> void
func bind_external_function(
	...
	bind_external_function_general(func_name, method, lookahead_safe)
...
class ExternalFunctionDef extends InkBase:
	...
	func _init(method: Callable, lookahead_safe: bool):
		self.method = method
